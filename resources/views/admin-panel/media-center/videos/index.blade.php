...
@section('title', __('adminlte::menu.videos'))

@section('content_header')
    <h1>{{ __('adminlte::menu.videos') }}</h1>
@stop
...
@section('content')
    ...
    <div class="container mx-0  mb-5">
        <a href="{{ route('videos.create', ['locale' => app()->getLocale()]) }}">
        ...
        <a href="{{ route('categories.index', ['locale', app()->getLocale()]) }}">
            <x-adminlte-button class="btn-lg mb-10" type="reset" label="{{ __('adminlte::adminlte.categoryManage') }}" 
                theme="outline-info" />
        </a>
    </div>
    ...
    @php
        ...
        foreach ($posts as $post) {
            ...
            $btnEdit =
                '<a href="' .
                route('videos.edit', ['video' => $post->id, 'locale' => app()->getLocale()]) .
            ...
            $btnActivation =
                ...
                '</a></span>';
            $vidThumbnail = '<a href="'. $post->mediaOne->link .'" target="_blank"><img src=' . $post->mediaOne->thumbnailpath . ' height="100" width="200" /></a> ';
            $data[] = [
                // $counter++ ?? 'N/A',
                $post->id,
    ...
    {{-- AdminLTE DataTable --}}
    <x-adminlte-datatable id="table2" :heads="$heads" head-theme="dark" :config="$config"  hoverable
        with-buttons />
    ...
    <script>
        $(document).ready(function() {
            $('#modalCustom').on('show.bs.modal', function(event) {
                ...
                var deleteAction =
                    '{{ route('videos.destroy', ['locale' => app()->getLocale(), 'video' => '__ID__']) }}';
            ...
            $('#modalActivate').on('show.bs.modal', function(event) {
                ...
                // Assuming 'videos.activate' is a route for activating videos. Adjust as needed.
                var action =
