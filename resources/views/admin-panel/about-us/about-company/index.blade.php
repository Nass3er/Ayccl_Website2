...
@section('title', __('adminlte::adminlte.aboutCompany'))

@section('content_header')
    <h1>{{ __('adminlte::adminlte.aboutCompany') }}</h1>
@stop
...
@section('content')
    ...
    @php
        $heads = [
            ...
            ['label' => __('adminlte::adminlte.icon'), 'classes' => 'border border-white bg-olive', 'width' => 11],
            [
        ...
        foreach ($posts as $post) {
            ...
            $btnEdit =
                '<a href="' .
                route('about-company.edit', ['about_company' => $post->id, 'locale' => app()->getLocale()]) .
                '" class="btn btn-warning " data-toggle="tooltip" title=' .
                __('adminlte::adminlte.edit') .
                '><i class="fas fa-edit"></i>' .
                '</a>';
            if ($post->order != 0) {
                $icon = '<div class="d-flex justify-content-center align-items-center text-success">'.
                                     (string)svg( $post->postDetailOne->color , ['width' => '50', 'height' => '50'])->toHtml()  .'
                                </div>';
                $btnDelete =
        ...
        $config = [
            ...
            // 'serverSide' => true,
            // 'processing' => true,
            // 'ajax' => "{{ localizedRoute('about-company.index') }}",
            'language' => [
    ...
    <script>
        $(document).ready(function() {
            $('#modalCustom').on('show.bs.modal', function(event) {
                ...
                var deleteAction =
                    '{{ route('about-company.destroy', ['locale' => app()->getLocale(), 'about_company' => '__ID__']) }}';
            ...
            $('#modalActivate').on('show.bs.modal', function(event) {
                ...
                // Assuming 'about-company.activate' is a route for activating about-company. Adjust as needed.
                var action =
