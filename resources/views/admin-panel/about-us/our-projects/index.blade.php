...
@section('title', __('adminlte::menu.ourProjects'))

@section('content_header')
    <h1>{{ __('adminlte::menu.ourProjects') }}</h1>
@stop
...
@section('content')
    ...
    <div class="container mx-0  mb-5">
        <a href="{{ route('our-projects.create', ['locale', app()->getLocale()]) }}">
    ...
    @php
        ...
        foreach ($posts as $post) {
            ...
            $btnEdit =
                '<a href="' .
                route('our-projects.edit', ['our_project' => $post->id, 'locale' => app()->getLocale()]) .
                '" class="btn btn-warning" data-toggle-disabled="tooltip" title="' . __('adminlte::adminlte.edit') . '"' .
                '><i class="fas fa-edit"></i>' .
                '</a>';
            $btnDelete =
                '<span style="display: inline-block;" data-toggle-disabled="tooltip" title="' . __('adminlte::adminlte.delete') . '"><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modalCustom" data-id="' .
                $post->id .
                '"'  .
            ...
            $btnActivation =
                '<span data-toggle-disabled="tooltip" title="' . __('adminlte::adminlte.activate') . '"><a href="#" class="btn btn-success" data-toggle="modal" data-target="#modalActivate" data-id="' .
                $post->id .
                '">' .
        ...
        $config = [
            ...
            // 'serverSide' => true,
            // 'processing' => true,
            // 'ajax' => "{{ localizedRoute('our-projects.index') }}",
            'language' => [
    ...
    <script>
        $(document).ready(function() {
            $('#modalCustom').on('show.bs.modal', function(event) {
                ...
                var deleteAction =
                    '{{ route('our-projects.destroy', ['locale' => app()->getLocale(), 'our_project' => '__ID__']) }}';
            ...
            $('#modalActivate').on('show.bs.modal', function(event) {
                ...
                // Assuming 'our-projects.activate' is a route for activating our-projects. Adjust as needed.
                var action =
