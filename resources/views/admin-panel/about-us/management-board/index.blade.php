...
@section('title', __('adminlte::adminlte.managementBoard'))

@section('content_header')
    <h1>{{ __('adminlte::adminlte.managementBoard') }}</h1>
@stop
...
@section('content')
    ...
    <div class="container mx-0  mb-5">
        <a href="{{ route('management-board.create', ['locale'=> app()->getLocale()]) }}">
    ...
    @php
        ...
        foreach ($posts as $post) {
            ...
            $btnEdit =
                '<a href="' .
                route('management-board.edit', ['management_board' => $post->id, 'locale' => app()->getLocale()]) .
        ...
        $config = [
            ...
            // 'serverSide' => true,
            // 'processing' => true,
            // 'ajax' => "{{ localizedRoute('management-board.index') }}",
            'language' => [
    ...
    <script>
        $(document).ready(function() {
            $('#modalCustom').on('show.bs.modal', function(event) {
                ...
                var deleteAction =
                    '{{ route('management-board.destroy', ['locale' => app()->getLocale(), 'management_board' => '__ID__']) }}';
            ...
            $('#modalActivate').on('show.bs.modal', function(event) {
                ...
                // Assuming 'management-board.activate' is a route for activating management-board. Adjust as needed.
                var action =
